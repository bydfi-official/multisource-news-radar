# Multisource News Radar

![Multisource News Radar cover](assets/multisource-news-radar-cover.png)

**Multisource News Radar** helps AI agents find, verify, rank, and summarize fresh public signals across news, feeds, social surfaces, research pages, and user-provided URLs.
...
```text
Install Multisource News Radar from this public repository:
https://github.com/BYDFi2025/multisource-news-radar

If you can persist skills in this environment:
1. Clone or download the repository.
2. Install it as a skill named multisource-news-radar in the appropriate skill directory for this agent.
...
```bash
python scripts/multisource_news_radar.py --out artifacts
```

Run a general hot-topic radar:

```bash
python scripts/multisource_news_radar.py --query "AI agents" --lookback-hours 24 --limit 25
```

Run a crypto and AI focused scan:

```bash
python scripts/multisource_news_radar.py \
  --query "crypto AI" \
  --query "agentic AI" \
  --mode news \
  --lookback-hours 12 \
...
python scripts/multisource_news_radar.py --query "open source AI browser automation" --source firecrawl
```

Generate automation and Telegram setup guides:

```bash
python scripts/multisource_news_radar.py --automation-guide --telegram-guide --out artifacts
```

Narrow the default briefing when needed:

```bash
python scripts/multisource_news_radar.py --region GB --language en-GB --out artifacts
python scripts/multisource_news_radar.py --query "AI policy" --mode news --out artifacts
```

Send a report to Telegram after setting `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID`:

```bash
python scripts/telegram_notify.py --file artifacts/<run>/radar-report.md --title "Multisource News Radar"
```
...
```bash
docker build --target final -t multisource-news-radar:base .
```
...
```bash
docker build --target browser -t multisource-news-radar:browser .
```
...
docker run --rm \
  -v "$PWD/artifacts:/app/artifacts" \
  multisource-news-radar:browser \
  --url "https://example.com/live-page" \
  --browser-fallback playwright \
  --out artifacts
```
...
Multisource News Radar uses a source ladder instead of relying on one scraper:
...
For pages that need a logged-in session, Multisource News Radar creates `authenticated-session-guide.md`. This guide is meant to be followed by an AI agent and the user together:
...
```text
artifacts/multisource-news-radar-<timestamp>/
├── radar-report.md
├── candidates.json
├── candidates.csv
├── sources.json
