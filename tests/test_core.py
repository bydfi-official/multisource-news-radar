...
from scripts.multisource_news_radar import Candidate, CoverageGap, dedupe_and_score, parse_html_candidate
from scripts.multisource_news_radar import parse_datetime
from scripts.multisource_news_radar import DEFAULT_GLOBAL_HOT_QUERIES, apply_default_profile, build_parser, google_news_regions
from scripts.multisource_news_radar import main, needs_auth_guide, selected_sources, write_auth_session_guide
...
def test_telegram_message_chunks_and_title(tmp_path) -> None:
    ...
    message = load_message(report, "Multisource News Radar")
    parts = list(chunks(message, max_chars=3900))
    assert message.startswith("Multisource News Radar")
    assert len(parts) == 3
